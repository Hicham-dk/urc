...
import { getAllUser } from './listUserApi';
...
const GetUser: React.FC<GetUserProps> = ({ onUserClick }) => {
  ...
  return (
    <Paper elevation={6} sx={{ padding: 4, maxWidth: 420, width: '100%', borderRadius: 2, backgroundColor: '#fff' }}>
      <Typography variant="h6" textAlign='center' sx={{ fontWeight: 'bold', color: '#0000FF' }}></Typography>
      {loading ? (
          ...
          {usersList.length > 0 ? (
            usersList.map((user, index) => (
              <ListItemButton key={index} onClick={() => handleClick(user.userId, user.username)} sx={{ mb: 1 }}>
                ...
                <Typography variant="body2" textAlign='center' sx={{ color: '#0000FF' }}>Aucun contact trouvé.</Typography>
              )}
            </List>
          )}
